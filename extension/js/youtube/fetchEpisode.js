...
export function assertTrackBody(status, body) {
    if (status === 429 || /automated queries/i.test(body)) {
        throw new TrackUnavailableError("YouTube is refusing caption downloads from this network right now " +
    ...
    if (!/<(?:p|text)[\s>]/.test(body)) {
        throw new TrackUnavailableError("YouTube returned something that is not a caption track. " +
 ...
 */
export async function fetchEpisodeWith(fetcher, videoId) {
...
export function fetchEpisode(videoId) {
