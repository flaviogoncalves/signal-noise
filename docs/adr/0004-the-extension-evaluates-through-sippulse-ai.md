...
The extension no longer stops at the clipboard. It harvests the transcript, applies the `signal-noise` skill to it with a language model on SipPulse AI, and shows the verdict next to the video. The only thing the user configures is a SipPulse AI key.
...
**One provider, one model, and it is that model or nothing.** The model id is not hardcoded, because the catalog is per-organization: the extension lists the models the key can use and finds the one it runs on among them. If it is not there, the extension refuses. It never falls back to another model, not even a newer one of the same family — a summary's quality is attributable only if the model that wrote it is always the same one. The model is resolved again on every evaluation, so a renamed id heals itself instead of failing with an error that never says "save your key again".

**The interface does not name the model.** Which model runs is a product decision that may change, and a user has nothing to do with the name: nothing in the panel, its messages, or the README states it. When a key cannot use the model, the refusal says so without naming it, and what the key *could* see goes to the browser console for whoever is debugging. The code necessarily knows the model, and this repository is public, so this is discretion in the product, not secrecy.
