/**
 * Render the subset of Markdown a summary uses, as HTML. Pure.
 *
 * The input is model output over a stranger's transcript, so it is hostile
 * until proven otherwise: everything is escaped before any markup is added,
 * and the only attribute ever written is an `href` that must be http(s).
 */
export function renderMarkdown(markdown: string): string {
  const lines = markdown.replace(/\u0000/g, "").replace(/\r\n?/g, "\n").split("\n");
  const html: string[] = [];
  let paragraph: string[] = [];
  let list: { tag: "ul" | "ol"; items: string[] } | undefined;
  let quote: string[] = [];

  const flush = (): void => {
    if (paragraph.length) html.push(`<p>${inline(paragraph.join(" "))}</p>`);
    if (list) html.push(`<${list.tag}>${list.items.map((item) => `<li>${inline(item)}</li>`).join("")}</${list.tag}>`);
    if (quote.length) html.push(`<blockquote>${renderMarkdown(quote.join("\n"))}</blockquote>`);
    paragraph = [];
    list = undefined;
    quote = [];
  };

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]!;
    const heading = /^(#{1,6})\s+(.*)$/.exec(line);
    const bullet = /^\s*[-*+]\s+(.*)$/.exec(line);
    const numbered = /^\s*\d+[.)]\s+(.*)$/.exec(line);
    const quoted = /^>\s?(.*)$/.exec(line);

    if (line.trim() === "") {
      flush();
    } else if (heading) {
      flush();
      const level = heading[1]!.length;
      html.push(`<h${level}>${inline(heading[2]!)}</h${level}>`);
    } else if (/^\s*([-*_])(\s*\1){2,}\s*$/.test(line)) {
      flush();
      html.push("<hr>");
    } else if (isTableRow(line) && isTableDivider(lines[i + 1])) {
      flush();
      const rows: string[] = [];
      for (i += 2; i < lines.length && isTableRow(lines[i]!); i++) rows.push(lines[i]!);
      i--;
      html.push(table(line, rows));
    } else if (bullet || numbered) {
      const tag = bullet ? "ul" : "ol";
      if (list?.tag !== tag) flush();
      list ??= { tag, items: [] };
      list.items.push((bullet ?? numbered)![1]!);
    } else if (quoted) {
      if (paragraph.length || list) flush();
      quote.push(quoted[1]!);
    } else if (list && /^\s+\S/.test(line)) {
      // An indented line under a bullet continues it.
      list.items[list.items.length - 1] += ` ${line.trim()}`;
    } else {
      if (list || quote.length) flush();
      paragraph.push(line.trim());
    }
  }

  flush();
  return html.join("\n");
}

const isTableRow = (line: string): boolean => /^\s*\|.*\|\s*$/.test(line);
const isTableDivider = (line: string | undefined): boolean =>
  line !== undefined && /^\s*\|(\s*:?-+:?\s*\|)+\s*$/.test(line);
const cells = (row: string): string[] => row.trim().slice(1, -1).split("|").map((cell) => cell.trim());

function table(head: string, rows: string[]): string {
  const th = cells(head).map((cell) => `<th>${inline(cell)}</th>`).join("");
  const body = rows
    .map((row) => `<tr>${cells(row).map((cell) => `<td>${inline(cell)}</td>`).join("")}</tr>`)
    .join("");
  return `<table><thead><tr>${th}</tr></thead><tbody>${body}</tbody></table>`;
}

function escapeHtml(text: string): string {
  return text
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

const emphasis = (text: string): string =>
  text.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>").replace(/\*([^*\s][^*]*)\*/g, "<em>$1</em>");

/**
 * Code spans and links are lifted out before emphasis runs, so an asterisk or
 * underscore inside a URL or a flag name is never read as markup.
 */
function inline(text: string): string {
  const held: string[] = [];
  const hold = (html: string): string => `\u0000${held.push(html) - 1}\u0000`;

  const escaped = escapeHtml(text)
    .replace(/`([^`]+)`/g, (_, code: string) => hold(`<code>${code}</code>`))
    .replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (whole: string, label: string, href: string) =>
      /^https?:\/\//i.test(href)
        ? hold(`<a href="${href}" target="_blank" rel="noopener noreferrer">${emphasis(label)}</a>`)
        : whole,
    );

  return emphasis(escaped).replace(/\u0000(\d+)\u0000/g, (_, index: string) => held[Number(index)] ?? "");
}
