...
import type { FetchedEpisode } from "../youtube/playerResponse.js";
...
let harvested: { videoId: string; fetched: FetchedEpisode } | undefined;
/** The video the summary on screen is about — not necessarily the one last harvested. */
let evaluated: OpenVideo | undefined;
let summaryMarkdown = "";
...
function describeTrack({ warning, trackLanguage, trackIsAutoGenerated, episode }: FetchedEpisode): string {
...
async function harvestOnce(video: OpenVideo): Promise<FetchedEpisode> {
...
async function videoOrExplain(): Promise<OpenVideo | undefined> {
  const video = await activeVideo();
  if (!video) report("Open a YouTube video first — this only works on a watch page.", "bad");
  return video;
}
